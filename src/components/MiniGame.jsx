...
export default function MiniGame() {
  ...
  useEffect(() => {
    ...
    // Canvas 2D cannot resolve CSS custom properties: assigning an invalid
    // colour to fillStyle is silently ignored and the previous value sticks,
    // so 'var(--accent)' painted every segment in the background colour and the
    // whole game rendered invisible. Resolve the tokens to literals up front.
    const cs = getComputedStyle(document.documentElement)
    const token = (name, fallback) => cs.getPropertyValue(name).trim() || fallback
    const ACCENT = token('--accent', '#4c7dff')
    const ACCENT_RGB = token('--accent-rgb', '76, 125, 255')
    const FOOD = token('--accent-2', '#e8b44a')
    let g = newState()
    let timerId = null
    ...
    function draw() {
      ...
      // Dot grid
      ctx.fillStyle = `rgba(${ACCENT_RGB},0.10)`
      for (let x = 0; x < COLS; x++)
        for (let y = 0; y < ROWS; y++)
      ...
      g.snake.forEach((seg, i) => {
        ...
        ctx.fillStyle = i === 0 ? ACCENT : `rgba(${ACCENT_RGB},${alpha})`
